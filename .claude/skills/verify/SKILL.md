...
**Stale-build trap**: more than one `DerivedData/Peek-*` directory can exist (regenerating the project can mint a new hash), and the glob above then launches **two** Peek instances — one of them weeks old. Before opening, pick the newest binary:

```bash
stat -f "%Sm %N" ~/Library/Developer/Xcode/DerivedData/Peek-*/Build/Products/Debug/Peek.app/Contents/MacOS/Peek
```
...
- **Find the status item**: screenshot the menu bar and locate the "SAT 18"-style glyph visually. Do NOT rely on `CGWindowListCopyWindowInfo` filtered by owner "Peek" — on this machine (macOS 26) menu bar item windows all enumerate under **Control Center**'s PID, so Peek owns zero listed windows even while its item is visible. The user also runs **Ice** (menu bar manager), which parks hidden items at large negative X. **Re-locate the icon before every click** — its position shifts whenever other status items come and go.
